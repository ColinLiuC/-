...
namespace CoreSolution.WebApi.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public UserController(IUserService userService, IRoleService roleService, IPermissionService permissionService, IStreetService streetService)
        {
        ...
        [Route("register")]
        [HttpPost]
        public async Task<JsonResult> Register(InputRegisterModel inputRegisterModel)
        {
            ...
            {
                StreetId = inputRegisterModel.StreetId,
                StreetName = inputRegisterModel.StreetName,
                PostStationId = inputRegisterModel.PostStationId,
                PostStationName = inputRegisterModel.PostStationName,
            ...
            if (inputRegisterModel.RegisterType == RegisterType.Assign)
            {
                ...
                var userCreator = _userService.SingleOrDefault(m => m.Id == userId);
                userDto.CreatorUserId = userId;
                //  userDto.CreatorUser = userCreator;
        ...
        [Route("login")]
        [HttpGet]
        public async Task<JsonResult> Login(InputLoginModel inputLoginModel)
        {
        ...
        private async Task<JsonResult> DoLoginAsync(string userNameOrEmailOrPhone, string password)
        {
            ...
            if (result == LoginResults.Success)
            {
                ...
                await LoginManager.SaveCurrentUserPermissionsAsync(permissions.Select(i => i.TargetName).ToArray(), user.Id);
                LoginManager.ResetErrorLogin(LoginErrorTimes_Prefix, userNameOrEmailOrPhone);

                await LoginManager.SaveCurrentUserStreetId(user.StreetId, user.Id, token);
                await LoginManager.SaveCurrentUserStationId(user.PostStationId, user.Id, token);

                var users = new UserParam()
                {
                    Token = token,
                    userId = user.Id,
                    StreetId = user.StreetId != null ? user.StreetId.Value : default(Guid),
                    StreetName = user.StreetName,
                    PostStationId = user.PostStationId != null ? user.PostStationId.Value : default(Guid),
                    PostStationName = user.PostStationName,
                    UserType = user.UserType,
                    UserName = user.UserName,
                    RealName = user.RealName
                };
                var streetinfo = await _streetService.GetAsync(users.StreetId);
                if (streetinfo != null)
                {
                    users.centerStreetLat = streetinfo.Lat;
                    users.centerStreetLng = streetinfo.Lng;
                }
                return AjaxHelper.JsonResult(HttpStatusCode.OK, "成功", users);//200
            }
        ...
        [Route("getUsersPaged")]
        [HttpGet]
        public async Task<JsonResult> GetUsersPaged(Guid? streetId, Guid? postStationId, string key, int pageIndex = 1, int pageSize = 10)
        {
            var where = PredicateExtensions.True<User>();



            if (streetId != default(Guid) && (!string.IsNullOrWhiteSpace(postStationId.ToString())))
            {
                where = where.And(i => i.StreetId == streetId);
            }
            if (postStationId != default(Guid) && (!string.IsNullOrWhiteSpace(postStationId.ToString())))
            {
                where = where.And(i => i.PostStationId == postStationId);
            }
            if (!key.IsNullOrWhiteSpace())
            {
                where = where.And(i => i.UserName.Contains(key));
            }
            var result = await _userService.GetPagedAsync(where, i => i.CreationTime, pageIndex, pageSize, true);
            ...
            {
                return AjaxHelper.JsonResult(HttpStatusCode.OK, "成功", new ListModel<OutputUserModel>());
            }
        }
        ...
        [HttpPost]
        public async Task<JsonResult> UpdateUserInfo(InputUserModel inputUserModel)
        {
            if (inputUserModel.UserId.IsNullOrEmpty())
            {
            ...
            var userDto = _userService.Get(inputUserModel.UserId);

            userDto.StreetName = inputUserModel.StreetName;
            userDto.StreetId = inputUserModel.StreetId;
            userDto.PostStationName = inputUserModel.PostStationName;
            userDto.PostStationId = inputUserModel.PostStationId;
            userDto.UserName = inputUserModel.UserName;
            userDto.RealName = inputUserModel.RealName;
        ...
        [HttpPost]
        public async Task<JsonResult> UpdatePassword(Guid userid, string password, string confirmpassword)
        {
            ...
            {
                return AjaxHelper.JsonResult(HttpStatusCode.BadRequest, "确认密码不能为空");
            }
            if (password != confirmpassword)
            {
                return AjaxHelper.JsonResult(HttpStatusCode.BadRequest, "输入两次密码不匹配");
            }

            await _userService.UpdatePassword(userid, password);

            return AjaxHelper.JsonResult(HttpStatusCode.OK, "成功");
        }


        [Route("getUserStreetAndStation")]
        [HttpGet]
        public async Task<JsonResult> GetUserStreetAndStation(string token)
        {
            try
            {
                //根据token获取userid
                var userId = (await LoginManager.GetUserIdAsync(token)).GetValueOrDefault();
                //根据userid获取streetid and stationid
                var streetid = await LoginManager.GetCurrentUserStreetIdAsync(userId, token);
                var stationid = await LoginManager.GetCurrentUserStationIdAsync(userId, token);
                return AjaxHelper.JsonResult(HttpStatusCode.OK, "成功", new List<Guid?>() { streetid, stationid });
            }
            catch (Exception)
            {
                return AjaxHelper.JsonResult(HttpStatusCode.OK, "失败", new List<Guid?>() { null, null });
            }
        }
    }
}